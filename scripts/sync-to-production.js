...
// Simple script to sync papal data to production using the existing setup script
...
async function syncToProduction() {
  console.log('🔄 Syncing papal data to production...');
  
  try {
    // First, let's run the existing production setup to ensure basic structure
    ...
    // Now let's add some sample papal data to production
    console.log('👑 Adding sample papal data to production...');
    
    // Check if we already have papal data
    const existingPopes = await prisma.pope.findMany();
    
    if (existingPopes.length === 0) {
      console.log('📝 Creating sample popes...');
      ...
      console.log('✅ Sample popes created in production');
    ...
    console.log(`   - ${popes.length} popes`);
    console.log(`   - ${users.length} users`);
    console.log('   - All papal data with detailed biographies and references');
