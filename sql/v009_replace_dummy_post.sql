-- Ajay's Catholic Commentary - Replace Dummy Post SQL
-- Generated: 2025-10-10
-- Description: Replaces dummy post with complete Sunday Gospel reflection content

BEGIN;

-- ==============================================
-- REPLACE DUMMY POST WITH SUNDAY GOSPEL CONTENT
-- ==============================================

-- First, let's see what posts exist
SELECT 'Current posts in database:' as info;
SELECT id, title, slug, status, "publishedAt" 
FROM posts 
ORDER BY "createdAt" DESC;

-- Update the dummy post (assuming it's the most recent or has a generic title)
-- This will update the first post it finds that might be a dummy post
UPDATE posts 
SET 
...
WHERE id = (
  ...
  WHERE status = 'DRAFT' OR title ILIKE '%dummy%' OR title ILIKE '%test%'
  ORDER BY "createdAt" DESC 
  LIMIT 1
);
...
SELECT 'Updated post details:' as info;
SELECT 
  id, 
  title, 
  slug, 
